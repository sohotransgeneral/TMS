...
function R({ label, value, neg, bold }: { label: string; value: string; neg?: boolean; bold?: boolean }) {
  return (
    <tr className={bold ? "border-t-2 border-black font-bold" : ""}>
      <td className="py-1 pr-4 text-sm text-gray-600 print:text-gray-700">{label}</td>
      <td className={`py-1 text-right font-mono text-sm ${neg ? "text-red-600" : bold ? "text-black" : "text-gray-900"}`}>
        {value}
      </td>
    </tr>
  );
...
export default async function DriverReportPage({
  ...
  const data = await getDriverFinancialData(driver.id, driver.salaryPerKm, driver.commissionRate, period);
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 px-6 py-8 print:px-0 print:py-0">
      ...
      <div className="flex items-center justify-between print:hidden">
        <Link href={`/admin/drivers/${id}?period=${period}`} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="border-b-2 border-black pb-4">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground print:text-black">{fullName}</h1>
            <p className="text-sm text-muted-foreground print:text-gray-600">{driver.user.email}</p>
          </div>
          <div className="text-right">
            <div className="text-lg font-semibold text-foreground print:text-black">Raport Financiar</div>
            <div className="text-sm text-muted-foreground print:text-gray-600">{data.periodLabel}</div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          { label: "Km parcurși", value: `${Math.round(data.totalKm).toLocaleString("ro-RO")} km` },
          { label: "Salariu net", value: fmt(data.taxes.net) },
        ].map((k) => (
          <div key={k.label} className="rounded border border-gray-300 p-3 print:border-gray-400">
            <div className="text-xs text-gray-500 print:text-gray-600">{k.label}</div>
            <div className="text-lg font-bold text-foreground print:text-black">{k.value}</div>
          </div>
        ))}
      </div>
      ...
      {/* Loads table */}
      <div>
        <h2 className="mb-3 text-base font-bold text-foreground print:text-black">Curse Efectuate ({loads.length})</h2>
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {loads.length === 0 && (
              <tr>
                <td colSpan={7} className="py-4 text-center text-gray-500">Fără curse în această perioadă</td>
              </tr>
            )}
            {loads.map((l, i) => (
              <tr key={l.id} className={`border-b border-gray-200 ${i % 2 === 0 ? "" : "bg-gray-50 print:bg-gray-100"}`}>
            ...
            {loads.length > 0 && (
              <tr className="border-t-2 border-gray-400 font-bold">
                ...
                <td className="py-2 text-right tabular-nums">{Math.round(data.totalKm).toLocaleString("ro-RO")}</td>
                <td className="py-2 text-right tabular-nums">{fmt(data.revenue, data.currency)}</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="grid grid-cols-2 gap-8">
        {/* Deductions */}
        <div>
          <h2 className="mb-3 text-base font-bold text-foreground print:text-black">Cheltuieli & Deduceri</h2>
          <table className="w-full border-collapse">
            <tbody>
              ...
              {data.fuelCost > 0 && <R label={`Combustibil (${data.fuelCount} înreg.)`} value={`- ${fmt(data.fuelCost, data.currency)}`} neg />}
              ...
              {data.repairCost > 0 && <R label="Reparații" value={`- ${fmt(data.repairCost, data.currency)}`} neg />}
              {data.maintCost > 0 && <R label="Mentenanță" value={`- ${fmt(data.maintCost, data.currency)}`} neg />}
              ...
              {data.totalDeductions === 0 && <R label="Fără cheltuieli" value="—" />}
        ...
        {/* Salary & taxes */}
        <div>
          <h2 className="mb-3 text-base font-bold text-foreground print:text-black">Calcul Salariu & Taxe</h2>
          <table className="w-full border-collapse">
            <tbody>
              <R label={`Distanță parcursă`} value={`${Math.round(data.totalKm).toLocaleString("ro-RO")} km`} />
              <R label="Rată €/km" value={driver.salaryPerKm ? `${driver.salaryPerKm} €/km` : "—"} />
              ...
              <tr><td colSpan={2} className="py-1" /></tr>
              <R label="SALARIU NET DE PLATĂ" value={fmt(data.taxes.net)} bold />
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div className="border-t border-gray-300 pt-4 text-xs text-gray-500 print:text-gray-600">
        ...
        <p className="mt-1">Taxe calculate conform legislației române: CAS 25% + CASS 10% + Impozit venit 10% din (Brut − CAS − CASS).</p>
      </div>
