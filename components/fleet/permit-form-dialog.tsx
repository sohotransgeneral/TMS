...
import { createPermit, updatePermit, PERMIT_TYPES } from "@/actions/permits";
import type { ActionResult } from "@/lib/action-helpers";
import { Upload, Loader2, FileText, X, ShieldCheck, Plus, Pencil } from "lucide-react";
...
export function PermitFormDialog({
  ...
  useEffect(() => {
    if (!state) return;
    if (state.ok) {
      toast.success(state.message ?? (isEdit ? "Permit actualizat." : "Permit adăugat."));
      setOpen(false);
    } else {
          ...
          <form action={formAction} className="grid gap-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field name="type" label="Permit Type" required>
                <Select name="type" defaultValue={initial?.type ?? "OVERSIZE"} required>
                  {PERMIT_TYPES.map((t) => (
              ...
              <Field name="permitNumber" label="Permit Number">
                <Input name="permitNumber" defaultValue={initial?.permitNumber ?? ""} placeholder="e.g. OH-2026-00123" />
              </Field>
            </div>

            <Field name="jurisdiction" label="Jurisdiction / State / Route">
              <Input name="jurisdiction" defaultValue={initial?.jurisdiction ?? ""} placeholder="e.g. Ohio, I-80 corridor" />
            </Field>

            <Field name="description" label="Description (dimensions / limits)">
              <Input name="description" defaultValue={initial?.description ?? ""} placeholder="e.g. 14'6&quot; wide, 120ft long, 105,500 lbs" />
            </Field>

            <div className="grid gap-4 sm:grid-cols-2">
              <Field name="validFrom" label="Valid From">
                <Input name="validFrom" type="date" defaultValue={toDateInput(initial?.validFrom)} />
              </Field>
              <Field name="validTo" label="Valid To">
                <Input name="validTo" type="date" defaultValue={toDateInput(initial?.validTo)} />
              </Field>
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <Field name="cost" label="Permit Cost">
                <Input name="cost" type="number" step="0.01" defaultValue={initial?.cost ?? ""} placeholder="0.00" />
              </Field>
            ...
            <div className="grid gap-4 rounded-lg border bg-muted/30 p-4 sm:grid-cols-2">
              <FileUploadField label="Permit Document" name="permitImageUrl" initial={initial?.permitImageUrl} />
              <FileUploadField label="Invoice / Receipt" name="invoiceUrl" initial={initial?.invoiceUrl} />
            </div>

            <Field name="notes" label="Notes">
              <Textarea name="notes" rows={2} defaultValue={initial?.notes ?? ""} placeholder="Any additional notes..." />
            </Field>
...
export function EditPermitButton({ permit, truckId }: { permit: Permit; truckId: string }) {
  return (
    <PermitFormDialog
      truckId={truckId}
      initial={permit}
